...
import { isObject, isArray, isFunction, mergeWith, merge } from 'lodash';
import { storage, downLoadFile } from '@the-sea/utils';
...
export default class Ajax {
  ...
  constructor(baseURL = '', options = {}) {
    ...
    mergeWith(this.defaultOptions, options, (objValue, srcValue) => {
      ...
      if (isFunction(objValue)) return srcValue;
  ...
  async reauest(config, options) {
    try {
      ...
      // 其他数据走cache、onAfter
      this.setCache(res.config, res.data, options.cacheTime);
      // 返回onAfter执行的结果
      return await onAfter(res.data);
    } catch (error) {
  ...
  get(url, params = {}, options = {}) {
    ...
    return this.reauest(config, { loading, onAfter, cacheTime, thirdPartRequest, needHeaders });
  }
  ...
  upload(url, data = {}, options = {}) {
    ...
    return this.reauest(config, { loading, onAfter });
  }
  ...
  async downloadFile(url, data = {}, options = {}) {
    ...
    const res = await this.reauest(config, { loading });
